...
df1 = df.load("hdfs://master:9000/user/user/ratings.parquet") 
df2 = df.load("hdfs://master:9000/user/user/movie_genres.parquet") 
...
sqlString = \
	"SELECT * " + \
	"FROM " + \
	" (SELECT * FROM movie_genres LIMIT 100) as mg, " + \
	" ratings as r " + \
	"WHERE " + \
	" r.movie = mg.Movie_ID" 
